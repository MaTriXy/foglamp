...
Contexts layer: `wrap(ai, { context })` sets the default, `fog.with(context)`
binds on top of it, and a call-time `foglamp` option wins over both.
`metadata` maps merge across layers (inner keys win) instead of replacing each
other.
...
Vercel it keeps the invocation alive by loading `waitUntil` from
`@vercel/functions` when that package is installed — add it as a dependency
there. On other serverless platforms pass `waitUntil` in the config (e.g.
