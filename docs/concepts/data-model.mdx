...
| `customer` | object? | `{ id, name?, imageUrl? }` — the end-customer served (see [Customer](#customer)) |
...
A **customer** (`customer`) is the end-customer a call serves — the tenant or
user *your* app is acting on behalf of. It rolls cost up per customer (the
**Customers** card on the [Overview](/dashboard/overview)), so it's the building
block for usage-based pricing on top of Foglamp.

| Field | Type | Notes |
| --- | --- | --- |
| `id` | string | **required** — 1–128 chars, the stable grouping key. Unlike names, this is a high-cardinality identifier (one per customer), so a dynamic id is correct here |
...
`name` and `imageUrl` are display-only and may change over time (Foglamp keeps
the latest). Only `id` is denormalized onto spans for attribution; the name and
image live in a small dimension table. `customer` is optional and purely
