...
<Note>
  **Embeddings aren't captured yet.** The wire contract reserves an `embedding`
  span type, but neither SDK path currently emits one: the v7 collector
  instruments `generateText`/`streamText` (and the `wrap` path adds
  `generateObject`/`streamObject`), while `embed`/`embedMany` and `rerank` are
  not yet traced. Instrumenting an `embed` call today produces a trace with a
