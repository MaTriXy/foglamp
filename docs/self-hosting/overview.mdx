...
There is no Redis or external queue — the ingest write buffer is in-memory and
flushes to ClickHouse on an interval and on graceful shutdown.
...
- **Retention** — `FOGLAMP_SPANS_RETENTION_DAYS` (default 30) is applied via
  `ALTER TABLE … MODIFY TTL` on boot. Change it and restart to adjust.
